...
def get_prediction_for_row(predictor, row, optimal_parameters, strike_matrix, vola_matrix, df):
    ...
    prediction = [round(predictor(params= param_0, fwd=fwd_0, strike=strike, expiry=expiry_0),4) for strike in strikes_0]
...
def plot_prediction_vs_reality(predictor, row, optimal_parameters, strike_matrix, vola_matrix, df):
    bps = np.array(df.columns[3:])
    df_filtered = df.iloc[row,3:]
    vola_true = df_filtered.to_numpy()
    prediction, rmse = get_prediction_for_row(predictor, row, optimal_parameters, strike_matrix, vola_matrix, df)
